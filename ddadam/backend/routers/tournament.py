...
async def start_tournament_job(
    ...
    try:
        ...
        initial_meta["message"] = "Threadでジョブを開始しました..."
        initial_meta["updated_at"] = datetime.now().isoformat()
        with open(job_file, "w", encoding="utf-8") as f:
            json.dump(initial_meta, f, ensure_ascii=False, indent=2)
        ...
        def run_job():
            try:
                logger.info(f"🚀 Thread実行開始: job_id={job_id}")
                run_tournament_job(job_id, req.game_id, req.jba_credentials, req.generate_pdf)
                logger.info(f"✅ Thread実行完了: job_id={job_id}")
            except Exception as e:
                logger.error(f"❌ Thread実行エラー: {str(e)}", exc_info=True)
        
        thread = threading.Thread(target=run_job, daemon=True)
        thread.start()
        
        logger.info(f"✅ Thread開始完了: job_id={job_id}")
